...
unsigned int gtp_int_gpio;
 ...
 //-other_TP wangdongbo.wt,MODIFY,2016/5/5
...
static void goodix_ts_work_func(struct work_struct *work)
{
 ...
 //-other_TP wangdongbo.wt,MODIFY,2016/5/5
//#if GTP_ICS_SLOT_REPORT
if(gtp_ics_slot_report==1)	/* add by wangdongbo.wt */
{
...
static void gtp_parse_dt(struct device *dev)
{
	...
	/********************** add 2016.4.28 **************/
//	devm_gpio_request(dev, gtp_int_gpio, "goodix,irq-gpio");	
//	devm_gpio_request(dev, gtp_rst_gpio, "goodix,rst-gpio");	
}
...
//Add, wangdongbo.wt, 20160520, free pinctl
...
void parse_cmdline_for_gt915(void)
{
	...
	cmdline_tp = strstr(saved_command_line, "androidboot.mode=");		
 	if(cmdline_tp != NULL)
 	{
		...
		ret = strncmp(temp, "ffbm-01", strlen("ffbm-01"));
		if(ret==0)
		{
...
//Modify, hubing1.wt_20160818_for_factory_test_current
...
//    GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
   ...
   ts->client = client;/* add by wangdongbo.wt_20150503 */
   ...
   if(!err && ts->ts_pinctrl)         
   {
	...
	goto free_reset_irq_gpio;//Add, wangdongbo.wt_20160520_free gpio
    }
...
#if GTP_AUTO_UPDATE
//Modify, hubing1.wt_20160818_test_current_bootmode
    err = get_boot_mode(client);
    if(err == 0) // nomal_boot mode
    {
...
//Add, wangdongbo.wt_20160520, free gpio
free_reset_irq_gpio:
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
    ...
    //guojunbo.wt modefy for suspend be called sometimes satrt 
	if (1 == ts->gtp_is_suspend)
	{
		return;
	}
	//guojunbo.wt modefy for suspend be called sometimes end
...
//Modify,@wangdongbo.wt_20160612_to_solve_suspend_fail
#if GTP_GESTURE_WAKEUP
